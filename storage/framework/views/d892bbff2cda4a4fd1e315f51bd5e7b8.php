...
<?php $__env->startSection('page-breadcrumb', 'Manajemen Konten'); ?>
        ...
        <div class="flex items-start gap-4 w-full sm:w-auto">
            ...
            <div class="min-w-0 flex-1">
                <div class="flex items-center gap-2 mb-1 flex-wrap">
                    <span class="px-2 py-0.5 rounded-md text-[0.7rem] font-bold border <?php echo e($typeColor); ?>">
                        <?php echo e($ann->type); ?>

                    </span>
                    <span class="text-[0.7rem] text-gray-400 font-medium">Target: <?php echo e($ann->target); ?></span>
                </div>
                ...
                <div class="text-[0.7rem] text-gray-400 mt-2 flex items-center gap-3">
                    <span>📅 <?php echo e($ann->start_date ? \Carbon\Carbon::parse($ann->start_date)->format('d M') : 'Sekarang'); ?> - <?php echo e($ann->end_date ? \Carbon\Carbon::parse($ann->end_date)->format('d M Y') : 'Seterusnya'); ?></span>
                </div>
            </div>
        </div>

        <div class="flex gap-1.5 shrink-0 justify-end pt-3 sm:pt-0 border-t sm:border-t-0 border-gray-100">
            <button class="btn btn-ghost bg-gray-50 hover:bg-blue-50 text-blue-600 border border-gray-200 btn-sm px-3 btn-edit" data-item="<?php echo e(json_encode($ann)); ?>">✏️ Edit</button>
            <form action="<?php echo e(route('admin.pengumuman.destroy', $ann->id)); ?>" method="POST" onsubmit="return confirm('Hapus pengumuman ini?')">
...
<div class="modal-overlay fixed inset-0 bg-black/50 backdrop-blur-sm z-50 hidden items-center justify-center p-4" id="modal-pengumuman">
    <div class="modal bg-white rounded-2xl p-7 w-full max-w-lg relative">
        ...
        <form id="form-pengumuman" action="<?php echo e(route('admin.pengumuman.store')); ?>" method="POST">
            <?php echo csrf_field(); ?>
...
<script>
    ...
    function prepareAdd() {
        ...
        form.reset();
        form.action = "<?php echo e(route('admin.pengumuman.store')); ?>";
    ...
    document.querySelectorAll('.btn-edit').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            document.getElementById('method-field').innerHTML = `<?php echo method_field('PUT'); ?>`;
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\kspm-sisfor\zxcvb\resources\views/admin/pengumuman.blade.php ENDPATH**/ ?>